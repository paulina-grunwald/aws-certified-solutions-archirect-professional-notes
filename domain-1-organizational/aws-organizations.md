...
- **"All accounts" ≠ management account** — SCPs/RCPs do NOT apply to the management account; restricting it requires IAM policies within that account
- **Don't confuse SCPs with IAM policies** — SCPs set the **maximum boundary**; they don't grant anything. An SCP allowing `ec2:*` does NOT give EC2 access — the account still needs an IAM policy that grants it
- **Don't confuse RCPs with resource-based policies** — resource-based policies are per-resource and set by the account owner; RCPs are organization-wide guardrails that **restrict** what resource-based policies can allow
- **Don't confuse declarative policies with SCPs** — SCPs filter IAM API calls; declarative policies configure service behavior directly. SCPs to enforce IMDSv2 require careful crafting; declarative policies just enforce it
- **RI/SP sharing is ON by default** — management must explicitly turn it off. If account A bought RIs but account B is benefiting, that's default behavior, not misconfiguration
- **SCP inheritance trap** — root SCP allows `*` but OU SCP allows only `s3:*` → effective permission is only `s3:*` (intersection). BOTH must allow the action
- **Service-linked roles bypass SCPs** — if a service still acts despite SCP deny, suspect a service-linked role
- **"Least privilege for management account"** — workloads or deployed resources in the management account = anti-pattern. Move them to member accounts and delegate admin
- **Consolidated billing ≠ All Features** — consolidated-billing-only mode has NO SCPs, RCPs, or policy-based controls. Governance scenarios require All Features mode. And you **cannot switch back** once enabled
