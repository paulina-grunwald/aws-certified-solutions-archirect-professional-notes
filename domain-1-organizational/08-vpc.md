...
- VPC peering is **not transitive** — chaining peering connections for A-to-B-to-C routing does not work
- Gateway Endpoints are **VPC-only** — they cannot be accessed from VPN, DX, peering, or TGW. Only Interface Endpoints support external access
- NAT Gateway is **AZ-scoped** — a single NAT Gateway is not highly available. Deploy one per AZ
- Security groups support **allow rules only** — if the question requires blocking specific IPs, you need NACLs
- NACLs are **stateless** — forgetting to allow ephemeral ports (1024-65535) on outbound/inbound will break traffic
- Interface Endpoints have **hourly + data charges** — Gateway Endpoints (S3, DynamoDB) are free. They are not equivalent in cost
- PrivateLink requires **NLB or GWLB** — if an answer says "ALB + PrivateLink," it is wrong
- TGW inter-Region peering uses **static routes only** — dynamic route propagation across Regions is not supported
- VPC Encryption Controls require **Nitro-based instances** — older instance types do not support hardware encryption
- VPC Lattice operates at the **application layer (L7)** — for L3/L4 inspection, use Network Firewall instead
