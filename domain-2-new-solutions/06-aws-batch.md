...
- Choose **EKS** (Batch on EKS) for Kubernetes-native batch
...
- **BEST_FIT** (default for legacy) — picks the least expensive instance type that fits the job. May queue jobs if that instance type isn't available. Newly-created BEST_FIT CEs default to launch templates.
...
- **SPOT_PRICE_CAPACITY_OPTIMIZED** — balances Spot price and capacity depth. Recommended over SPOT_CAPACITY_OPTIMIZED for cost-conscious Spot workloads.
...
- **Queue and Share Utilization Visibility** — Batch surfaces capacity used by FIFO + fair-share queues per allocation, in job queue snapshots
...
- **Queue Snapshots** + **Share Utilization Visibility** — for analyzing queue throughput
...
- Multi-Node Parallel Jobs require **On-Demand instances** — Spot Instances are not supported because the jobs need guaranteed nodes
- AWS Batch supports **any container with no time limit** — Lambda has a 15-min hard limit. They are not interchangeable for long-running jobs
- BEST_FIT can **stall the queue** if the cheapest instance type is unavailable — use BEST_FIT_PROGRESSIVE or SPOT_CAPACITY_OPTIMIZED for resilience
- Fargate has a **max 16 vCPU and 120 GB memory** per task — for larger jobs, use EC2
- Job definitions are **versioned** — updating creates a new revision, and jobs reference a specific version
- The execution role lets ECS **pull images and push logs**; the job role lets your code **call AWS APIs** — they are separate roles with different purposes
- The current name is **EventBridge** — "CloudWatch Events" is the old name for the same service
- Array job indexes are passed via the **`AWS_BATCH_JOB_ARRAY_INDEX` environment variable** — don't pass them as command-line arguments
- A job's queue is **fixed after submission** — to change queues, you must terminate and resubmit
- Compute environments are **Region-scoped** — multi-Region batch processing requires separate environments per Region
- Private subnets must have a **route to ECS** (NAT GW or VPC Endpoints for ECS + ECR + Logs + S3) — this is a common networking pitfall
- Batch is **queue-based** and not suitable for real-time / synchronous request-response — use Lambda, App Runner, or ECS Service for that
- Batch **runs the jobs**; Step Functions **orchestrates the workflow** — they are complementary, not interchangeable
- Job retries have a **max of 10 attempts** — design for idempotency
- Spot interruption is **not treated as failure by default** — configure the retry strategy to retry on `SpotInterruption` reason but not on application errors
