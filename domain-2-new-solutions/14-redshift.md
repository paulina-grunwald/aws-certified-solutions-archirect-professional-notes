...
- **Auto and incremental refresh on data sharing consumer tables** — consumers can have MVs that incrementally refresh from shared base tables
...
- **Governed by Lake Formation** — apply column / row security to shared data
...
- **DynamoDB → Redshift**
- **Salesforce → Redshift**
- **Multi-AZ deployment** supported with zero-ETL
...
- **Trusted Identity Propagation (TIP)** with IAM Identity Center — propagate user identity from Identity Center into Redshift queries, including Redshift Data API
...
- **Multi-AZ on RA3** gives you HA within Region — older notes saying "Single-AZ only" are obsolete
...
- Redshift supports **Multi-AZ on RA3** — "Single-AZ only" is obsolete information
- Data sharing requires **RA3 or Serverless** — DC2 does not support data sharing
- **RA3 or Serverless** are the current-generation options — DC2 and DS2 are legacy and should not appear in correct answers for new designs
- Spectrum, Federated Query, Data Sharing, and Zero-ETL are **four distinct patterns** — don't conflate them:
  - **Spectrum** = Redshift queries S3
  - **Federated Query** = Redshift queries Aurora / RDS live
  - **Data Sharing** = Redshift-to-Redshift live sharing
  - **Zero-ETL** = Aurora / RDS / DDB continuous replication into Redshift
- Redshift is **OLAP only** — columnar storage is unsuitable for single-row CRUD (OLTP). Use Aurora / RDS / DynamoDB for transactional workloads
- The leader node is **free of charge** — only compute nodes are billed. Questions may try to trick on this
- Redshift uses **sort keys, distribution keys, zone maps, and compression** for performance — it does not use indexes
- Cluster encryption can be **enabled transparently** on an unencrypted cluster — no manual snapshot restore is required
- Spectrum charges per **TB scanned**, not per query — use partitioning, Parquet, and compression to reduce cost
- Streaming Ingestion writes **directly** to Redshift (no S3 staging) — the old pattern of Firehose-to-S3-then-COPY is no longer needed
- Zero-ETL handles **replication only** — transformations and business logic still require Glue / Lambda / dbt downstream
- Federated Query supports **PostgreSQL and MySQL only** (Aurora + RDS) — it does not support MongoDB or DynamoDB
- Concurrency Scaling provides **1 hour free per day** per cluster — beyond that, you pay per second. It is not unlimited free
- Redshift Serverless **supports data sharing** as both producer and consumer — don't rule it out
- Cross-account data sharing uses **RAM under the hood** and requires no networking changes — VPC peering is not needed
...
| HA within Region                        | Multi-AZ on RA3                            |                                  |
...
| Per-user audit                          | TIP + IAM Identity Center                  |                                  |
